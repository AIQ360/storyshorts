...
const CONTENT_WIDTH = BASE_WIDTH - 2;
...
export function Safari({
  ...
  // If image dimensions provided, compute height to match aspect ratio (no cropping)
  const contentHeight =
    imageWidth && imageHeight
      ? Math.round(CONTENT_WIDTH * (imageHeight / imageWidth))
      : DEFAULT_HEIGHT - TOOLBAR_HEIGHT;
  const H = TOOLBAR_HEIGHT + contentHeight;

  // viewBox handles scaling — no explicit width/height so CSS controls sizing
  return (
    <svg
      aria-label="Safari browser"
      fill="none"
      role="img"
      viewBox={`0 0 ${BASE_WIDTH} ${H}`}
    ...
    >
      <g clipPath="url(#path0)">
        <path
          ...
          d={`M0 52H1202V${H - 12}C1202 ${H - 5.373} 1196.63 ${H} 1190 ${H}H12C5.37258 ${H} 0 ${H - 5.373} 0 ${H - 12}V52Z`}
        />
        <path
          className="fill-[#E5E5E5] dark:fill-[#404040]"
          clipRule="evenodd"
      ...
      <defs>
        <clipPath id="path0">
          <rect fill="white" height={H} width={BASE_WIDTH} />
        </clipPath>
        <clipPath id="roundedBottom">
          <path
            d={`M1 52H1201V${H - 13}C1201 ${H - 6.925} 1195.92 ${H - 1} 1190 ${H - 1}H12C5.92486 ${H - 1} 1 ${H - 6.925} 1 ${H - 13}V52Z`}
            fill="white"
          />
        </clipPath>
      </defs>
