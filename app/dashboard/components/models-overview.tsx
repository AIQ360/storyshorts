...
const MAX_VISIBLE = 3; // show only 1 row (3 cards + 1 "new" card = 4 columns)
    ...
    try {
      const url =
        deleteScope === "selected"
          ...
          : "/api/astria/clear-models?type=ai-models";
      ...
      if (res.ok) {
        if (deleteScope === "selected") {
          ...
          setModels([]);
          toast.success(d("allModelsCleared"));
        }
        setSelected(new Set());
        setSelectMode(false);
      } else {
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-2 rounded-full bg-primary/5 px-4 py-2">
            ...
            <span className="text-xs text-muted-foreground">
              {credits !== 1 ? d("creditPlural") : d("credit")}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        ...
        {!selectMode && (
          ...
          >
            ...
            <p className="text-sm font-medium text-black">{d("newModel")}</p>
